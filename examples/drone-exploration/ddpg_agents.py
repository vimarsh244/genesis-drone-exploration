...
class DDPGAgent:
    ...
    def act(self, obs: torch.Tensor, add_noise=True):
        ...
        if add_noise and self.cfg.noise_std > 0:
            noise = torch.randn_like(a) * torch.tensor([0.1, 2.0, 0.2])
            a = a + noise
        # clip to bounds
    ...
    def update(self, batch):
        ...
        if self.cfg.multi_critic:
            ...
            critic_loss = self.loss_fn(q1, y1) + self.loss_fn(q2, y2) + self.loss_fn(q3, y3)
        else:
            q = self.critic(o, a)
            with torch.no_grad():
        ...
        return dict(critic_loss=critic_loss.item(), actor_loss=act_loss.item())
